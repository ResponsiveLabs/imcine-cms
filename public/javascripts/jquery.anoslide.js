...
(function () 
{
    ...
    anoSlide.prototype = 
    {
			...
			if (!this.progress)
			{
				...
				$.each((reverse ? this.slides.reverse() : this.slides), delegate(this, function(key, slide)
				{
					...
					setTimeout(delegate(this, function()
					{
						slide.element.css(
						{
							width: parseInt(this.element.width()/this.options.items) + 'px',
							height: 'auto'
				...
				{
					height: this.slides[index].element.outerHeight()
				});
		...
		adapt: function()
		{
			...
			this.element.css(
			{
				height: this.slides[this.current].element.outerHeight()
			})
